...
class PartDataset(data.Dataset):
	def __init__(self, root=dataset_path, npoints=2500, classification=False, class_choice=None, split='train',
		...
		self.prior_set = get_prior_set('./data/compare_lib/' + class_choice + '/')
	...
	def __getitem__(self, index):
		...
		# prior
		prior, idx = get_prior(point_set, self.prior_set)
		# Feb 24 final try
		# prior = resample_pcd(prior, 512)
		prior = resample_pcd(prior, 2048)
		
		if self.classification:
			
			return point_set, cls, prior
		else:
